(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("MoveOver", [], factory);
	else if(typeof exports === 'object')
		exports["MoveOver"] = factory();
	else
		root["MoveOver"] = factory();
})(this, function() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__(/*! /home/zhirzh/JS/move-over/src/index.js */298);
...
/***/ function(module, exports) {
	...
	function renderPath() {
	  ...
	  for (var _i = 1; _i < points.length; _i += 1) {
	    var d = Math.sqrt(Math.pow(points[_i].x - points[_i - 1].x, 2) + Math.pow(points[_i].y - points[_i - 1].y, 2));
	
	    var delta = now - points[_i].t;
	    var alpha = Math.max(1 - delta / 1000, 0);
	
	    ctx.beginPath();
	    ctx.strokeStyle = 'rgba(0,0,0,' + alpha + ')';
	    ctx.lineWidth = 30 + 70 * Math.max(1 - d / 100, 0);
	    ctx.moveTo(points[_i - 1].x, points[_i - 1].y);
	    ctx.lineTo(points[_i].x, points[_i].y);
	    ctx.stroke();
	  }
	...
	function loadImage(src) {
	  var image = new Image();
	  image.src = src;
	  image.promise = new Promise(function (res) {
	    image.onload = res;
	  });
	...
	function init(_ref) {
	  ...
	  var _canvas = _ref$canvas === undefined ? createCanvas() : _ref$canvas;
	  ...
	  Promise.all([img.promise, imgB.promise]).then(onLoaded).then(resize).then(render).then(bindEventListeners);
	}
